...
- Python/Pandas：基于 `data/` 目录中的 mock CSV 做数据清洗和汇总，当前主脚本为 `scripts/erp_data_cleaning_demo.py`
...
## Python/Pandas 数据清洗扩展

当前已进入作品链第二阶段：Python/Pandas 数据清洗 Demo。
...
这个 Demo 证明我不只是会搭表格，也能把业务规则转化成可重复运行的数据处理流程，为后续 RPA 和 AI 工作流打基础。
```
